...
<figure className="w-[min(880px,calc(100vw-3rem))] mx-[calc((100%-min(880px,calc(100vw-3rem)))/2)]">
  ...
  <figcaption className="font-sans text-ink-muted text-[14px] tracking-[0.01em] leading-[1.6] text-left mt-[var(--figure-gap-caption)] [text-wrap:pretty]">
    上:108 个岗位的判断总量与全链路耗时——耗时按台账时间戳分段统计,间隔超过 15 分钟即切为新时段,共 7 段。下:单次判断本身的用时,不含贴入 JD、等待解析与归档。 / Top: totals across 108 verdicts; elapsed time measured from log timestamps, split into sessions at gaps over 15 minutes — seven sessions in all. Bottom: time for the verdict step itself, excluding JD intake and archiving.
  </figcaption>
</figure>
