...
<WorkImage
  ...
  caption="单个岗位从 JD 文本到归档的完整链路。模型负责解析与五维度判断，输出固定 10 字段的决策卡；是否投递由人决定。双轨留存中，Markdown 版本供回看，`decisions_log.csv` 台账供后续统计。 / The full path a single posting takes, from raw JD text to archive. The model handles parsing and the five-dimension judgment, producing a decision card with ten fixed fields; whether to apply is decided by the person. Of the two archives, Markdown is for re-reading and `decisions_log.csv` feeds later statistics."
