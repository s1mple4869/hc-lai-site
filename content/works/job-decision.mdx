<ProjectHeader
  ...
  titleCn="半自动化 JD 决策工作流"
  year="2026"
/>

<Hero
  src="/works/hero/hero-job-decision.webp"
  alt="纸质拼贴：一叠招聘启事卡片被分成三摞，居中一摞立起并盖有陶土色火漆印，另两摞平摊搁置，一副眼镜与一支铅笔置于纸堆旁，示意逐份审阅与决断。"
/>

## 01 | 项目概述 Project Summary

一个围绕**高频岗位投递决策**这一真实痛点搭建的半自动化工作流。通过 Claude API 实时解析岗位 JD，匹配三版差异化简历与招呼语版本，输出结构化「投递决策卡」。
...
<WorkImage
  src="/works/job-decision/pipeline-job-decision.svg"
  caption="单个岗位从 JD 文本到归档的完整链路。模型负责解析与五维度判断，输出固定 10 字段的决策卡；是否投递由使用者决定。双轨留存中，Markdown 版本供回看，`decisions_log.csv` 台账供后续统计。 / The full path a single posting takes, from raw JD text to archive. The model handles parsing and the five-dimension judgment, producing a decision card with ten fixed fields; whether to apply is decided by the person. Of the two archives, Markdown is for re-reading and `decisions_log.csv` feeds later statistics."
  alt="流程图：竖排六个阶段——JD 文本输入、实时解析（Claude API）、五维度判断、版本匹配、结构化决策卡（以强调色标出）、本地双轨留存；每个阶段右侧列出对应细节说明。"
  placeholder={false}
/>
...
<WorkImage
  src="/works/job-decision/paths-job-decision.svg"
  caption="三条技术路径的先后尝试与结果。接口自动化与浮窗注入分别撞上平台风控与反调试检测，命令行半自动成为当前主流程。两条废弃路径各有留存——wrapper 的 STOP 机制与浮窗架构都被保留了下来。 / Three technical paths, tried in order. API automation hit the platform's risk control and the browser overlay hit anti-DevTools detection; the CLI semi-automatic path became the main pipeline. Neither dead end was wasted — the wrapper's STOP mechanism and the overlay architecture were both kept."
  alt="路线图：并列三条技术路径——接口自动化（标注废弃）、浮窗注入（标注废弃）、命令行半自动（标注当前主流程，结果节点以强调色标出）；底部注释区说明两条废弃路径均保留原始记录。"
  placeholder={false}
/>
...
<WorkImage
  src="/works/job-decision/decision-card.png"
...
<figure className="w-[min(880px,calc(100vw-3rem))] mx-[calc((100%-min(880px,calc(100vw-3rem)))/2)]">
  <div className="flex flex-col gap-4">
    <StatRow
      stats={[
        { value: "108", label: "岗位判断总数" },
        { value: "3h41m", label: "全链路累计耗时" },
        { value: "10×", label: "效率提升倍数", accent: true },
      ]}
    />
    <CompareBars
      before={{ label: "人工判断", value: "5 min / 岗位" }}
      after={{ label: "工作流判断", value: "30 s / 岗位" }}
      ratio={10}
    />
  </div>
  <FigureCaption caption="上:108 个岗位的判断总量与全链路耗时——耗时按台账时间戳分段统计,间隔超过 15 分钟即切为新时段,共 7 段。下:单次判断本身的用时,不含贴入 JD、等待解析与归档。 / Top: totals across 108 verdicts; elapsed time measured from log timestamps, split into sessions at gaps over 15 minutes — seven sessions in all. Bottom: time for the verdict step itself, excluding JD intake and archiving." />
</figure>
