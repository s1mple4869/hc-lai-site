...
<Hero
  src="/works/hero/hero-ai-workflow.webp"
  alt="纸质拼贴：左侧散落揉皱的碎纸与便签，穿过中央一册牛皮纸夹（带陶土色书签），右侧汇为一摞码放整齐的卡片，示意从零散信息到结构化输出。"
/>
...
<WorkImage
  src="/works/ai-workflow/mechanism-ai-workflow.svg"
  caption="四类混乱输入共用同一套 A–E 输出结构，结构一致但按场景差异化扩展。底部六条治理原则不是流程中的一步，而是约束整个生成过程的规则层——其中「对外使用前必须人工审核」同时也是流程末端的一道闸门。 / Four kinds of messy input share one A–E output structure — consistent in shape, extended differently per scenario. The six governance rules at the bottom are not a step in the flow but a constraint layer over the whole generation; one of them — human review before any external use — is also the flow's final gate."
  alt="机制图：四类输入分别汇入同一套 Schema（A 至 E，按场景差异化扩展），最终统一进入人工审核环节（以强调色标出）；底部列出六条治理原则，中英对照单列排布。"
  placeholder={false}
  disableNativeWidthCap
/>

Schema 中的模块 B「可追踪任务表」，在两类输入下的实际输出如下。

Below are the actual outputs of Schema block B — the trackable task table — under two kinds of input.
...
<figure className="w-[min(880px,calc(100vw-3rem))] mx-[calc((100%-min(880px,calc(100vw-3rem)))/2)]">
  ...
  <FigureCaption caption="同一套 schema 在两类输入下的不同形态。上:由甲方口头反馈生成的任务表,负责人未定时写 TBD、日期未定时留空而非填入「下周」。下:由需求文档生成的检查清单,列名整体改为 Item / Status / Notes / Priority——它回答的不是「谁在什么时候做什么」,而是「哪些信息还缺」。节选 6 条,完整清单共 13 条。两表分别取自 Case 01(甲方变更)与 Mini Case 02(需求收集)。 / The same schema, two shapes. Top: a task table generated from verbal client feedback — Owner falls back to TBD, and Due is left blank rather than filled with 'next week.' Bottom: a checklist generated from a requirements document, where the columns shift to Item / Status / Notes / Priority — it answers not 'who does what by when' but 'what information is still missing.' Six of 13 rows shown. Drawn from Case 01 (Client Change) and Mini Case 02 (Requirement Intake)." />
</figure>

## 04 | 场景验证 Scenarios

这套 workflow 在 4 类典型场景下做了结构验证，input 为模拟样本。每个 case 包含一段 messy input 与完整的 A–E 输出，验证的是 schema 能否承接不同类型的混乱输入，而不是真实项目中的产出效果。

The workflow was structurally validated across four representative scenario types, using simulated input. Each case pairs one stretch of messy input with a full A–E output — what this tests is whether the schema holds across input types, not how it performed on a live project.

<CaseExpandable
  title="Case 01 | Client Change · 甲方变更"
  scenario="把一份模糊的甲方设计变更要求,转译为可汇报的执行更新和可追踪任务清单。"
  whyItMatters="外部、非结构化的甲方反馈最容易丢掉行动项和风险——这一例把它转成有行动、有风险、可原句溯源的结构化输出。"
  whatItProves={[
...
<CaseExpandable
  ...
  whyItMatters={'同一套 workflow 换一种输入类型（领导请求，而非甲方变更），并叠加"外企风格"的语言约束。'}
  whatItProves={[
...
<CaseExpandable
  ...
  whyItMatters={'很多团队真正的问题不是"有没有开会"，而是"会后到底谁做什么"——这一例处理的是口语化的对话型输入，而非结构化请求。'}
  whatItProves={[
...
<CaseExpandable
  ...
  whyItMatters={'很多流程问题不是从聊天、而是从一份模糊文档开始的——这一例处理的是文档型输入：brief、需求说明、半成型文档。'}
  whatItProves={[
