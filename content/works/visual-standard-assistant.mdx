...
<Hero
  src="/works/hero/hero-visual-standard.webp"
  alt="纸质拼贴：一枚黄铜放大镜压在一张护肤品主图照片上，放大其中一角，右侧排列着几个勾选框，示意逐条对标视觉标准。"
/>
...
我还引入了一条<strong>「视觉说服范式」判别轴(功效驱动 / 品牌驱动 / 混合)</strong>,让系统判的是「这张图<strong>靠什么</strong>说服用户」,而不只是数「有没有标题、有没有产品」——避免把高端品牌感误当成功效型主图的加分项。
...
<WorkImage
  src="/works/visual-standard-assistant/workflow-visual-standard-v5.svg"
  caption="Dify 双模式工作流全景:以「有无上传图片」分流为判图 / 问答两条路径,两条路径共用同一知识库;判图路径先做「只观察、不判断」的视觉分析,再进入检索与判定。 / The Dify workflow branches on image upload into judge-image and Q&A paths, both drawing on the same knowledge base; the judge-image path first runs a describe-only visual analysis before retrieval and verdict."
  alt="流程图：用户输入经条件分支分流——无图走文本路径（知识检索 → 文字回答 → 输出 1），有图走图文路径（视觉分析 → 知识检索 → 图片判断 → 输出 2）；下方虚线注释记录两次误判的失败复盘。"
  placeholder={false}
  wide
/>
...
<div className="font-mono text-ink-muted text-[10px] tracking-[0.08em] text-center mt-10 mb-8 px-6 md:px-0 leading-[1.8]">
  以下案例图为原创 mock 图，品牌与商品均为虚构，仅用于展示系统的判断逻辑。<br />
  Mock images below are original; brands and products are fictional, shown only to illustrate the system's judgment logic.
</div>

<WorkImage
  src="/works/visual-standard-assistant/mock-a-baby.png"
  caption="案例 A · 婴童护理 mock 主图（虚构品牌 NUBLOOM，AI 生成）——判图模式的测试样张：整图送入工作流，输出综合判定与逐项修改建议。 / Case A · an AI-generated mock baby-care hero image (fictional brand NUBLOOM) — test input for judge-image mode: the full composition goes in, an overall verdict and item-level fixes come out."
  placeholder={false}
/>

<JudgmentCard
  title="案例 A · 品类适配正例：婴童护理主图"
  verdict="high"
  coreReason="该图整体符合功效驱动型主图语法。虽然使用了粉色与婴童元素，但它们与婴童护理品类高度适配，并没有削弱功效表达。画面仍然保持了功效前置、产品突出、信任背书和底部转化信息的完整结构。"
  pros={[
    "功效大标题明确，能够快速传达产品定位与核心作用。",
    "问题 / 利益点以勾选形式呈现，直接对应用户可能遇到的护理场景。",
    "产品主体清晰，包装与产品组合占据主要视觉区域，识别度高。",
    "信任背书明确，如温和配方、敏感肌适用、渠道背书等信息增强可信度。",
    "底部转化 / 利益补充模块完整，能够进一步强化购买理由。"
  ]}
  consNote="基本没有明显跑偏，以下为可优化点："
  cons={[
    "可进一步增强标题与勾选列表之间的层级区分，让用户浏览时更快抓住重点。",
    "底部模块可适当减少信息密度，避免在小尺寸展示时可读性下降。"
  ]}
  reference="品类适配正例；婴童护理类功效型主图；「语法稳定，皮肤随品类变化」的标准样本。"
/>

<WorkImage
  src="/works/visual-standard-assistant/mock-b-serum.png"
  caption="案例 B · 抗老精华 mock 主图（虚构品牌 VELSAINE，AI 生成）——功效型正例：换一个品类验证同一套语法，判定为「高度接近」。 / Case B · an AI-generated mock anti-aging serum hero (fictional brand VELSAINE) — an efficacy-driven positive: the same grammar tested on a different category, judged “highly aligned.”"
  placeholder={false}
/>

<JudgmentCard
  title="案例 B · 功效型正例：抗老精华主图"
  verdict="high"
  coreReason="该图符合功效驱动型主图的核心结构：功效标题清晰、产品主体突出、问题 / 利益点明确、信任背书可见，并且具备底部利益总结。整体说服逻辑以「问题—功效—产品—背书—转化」为主，而不是单纯依赖品牌氛围。"
  pros={[
    "主标题直接表达「紧致抗皱」等核心功效，信息进入速度快。",
    "勾选式利益点清晰列出用户关心的问题，如细纹、松弛、轮廓、肤感等。",
    "产品主体占据主要视觉区域，包装清晰，主次关系明确。",
    "信任背书围绕功效建立，例如成分复配、实验验证等，能够增强可信度。",
    "底部利益条对核心卖点进行总结，强化转化导向。"
  ]}
  consNote="基本没有明显跑偏，以下为可优化点："
  cons={[
    "可进一步增强信任背书与产品之间的关联，让用户更快理解「为什么可信」。",
    "背景质感偏高级，可注意避免过度品牌化，保持功效信息优先。"
  ]}
  reference="功效型护肤主图正例；抗老精华类主图；医研感 / 成分功效型视觉表达。"
/>

<WorkImage
  src="/works/visual-standard-assistant/mock-c-poster.png"
  caption="案例 C · 高端海报型 mock 主图（虚构品牌 VELSAINE，AI 生成）——品牌驱动反例：靠品牌氛围而非功效说服，被范式判别轴识别为「部分接近」。 / Case C · an AI-generated mock premium poster-style hero (fictional brand VELSAINE) — a brand-driven negative: persuading with brand mood rather than efficacy, caught by the paradigm axis and judged “partially aligned.”"
  placeholder={false}
/>

<JudgmentCard
  title="案例 C · 品牌驱动反例：高端护肤海报型主图"
  verdict="partial"
  coreReason="该图产品清晰、画面高级、背景克制，但整体更偏品牌驱动型视觉。它主要依靠品牌氛围、产品质感和高端审美说服用户，缺少明确的问题 / 利益点勾选、功效型信任背书和强转化模块，因此不完全符合功效驱动型主图标准。"
  pros={[
    "产品主体清晰，包装识别度较高。",
    "背景克制，没有明显干扰产品展示。",
    "标题可读，能够传达一定的产品方向。"
  ...
  cons={[
    "缺少明确的问题 / 利益点勾选，用户无法快速理解产品解决什么具体问题。",
    "信任背书偏品牌氛围，缺少认证、成分、实验验证、渠道背书等功效型信任信息。",
    "底部模块较弱，未形成明确的购买引导或利益补充。",
    "整体更像品牌海报 / 形象图，而不是以功效转化为核心的信息型主图。"
  ]}
  reference="品牌驱动型反例；高端护肤海报型主图；范式不匹配样本。"
/>
...
v1.1 中新增「视觉说服范式」判别轴，将图像先区分为：功效驱动型 / 品牌驱动型 / 混合型 / 其他。同时加入两层判断模型：① <strong>稳定语法</strong>——功效标题、问题 / 利益点、产品主角、信任背书、底部转化模块；② <strong>品类皮肤</strong>——颜色、人物、气质、场景随品类变化。
